...
def renameFile(path, customName, start_num):
	...
	if len(start_num) < 1:
		start_num = 1
	x = int(start_num)
	newExt = os.path.splitext(customName)[1]
	for file in os.listdir(path):	
		...
		if newExt != oldExt and len(newExt) > 0:
			# print("Check the newExt with oldExt")
		...
		elif len(newExt) < 1 and len(oldExt) > 0:
			# print("When no Extension was provided but there was an Extension")
			newName = customName + " " + y + oldExt
			old = os.path.join(path, OldName)
			new = os.path.join(path, newName)
			...
			# print("When File had no Extension")
			newName = customName + " " + y + ".txt"
			old = os.path.join(path, OldName)
			new = os.path.join(path, newName)
...
createInstruction = """For Creating a file/No of Files in a directory\n Please Provide Name With a Extension\nIf no Extension is provided .txt files will be created \n 1 File will be created if no number is provided\n Default file name if file 01.txt"""

win = tk.Tk()
win.title("Rename Files")
win.geometry("275x372")
